...
class TestAlarmScore(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        with warnings.catch_warnings():
            ...
            self.env = make(self.env_nm, test=True,reward_class=_AlarmScore)#,param=param)

        self.env.seed(0)
        self.env.reset()
        self.env.reset()
        self.max_iter = 10
        self.default_kwargs_att_budget = {"max_budget": 3.,
    ...
    def test_reward_wrong_area_wrong_time(self):
        ...
        obs, reward, done, info = self.env.step(self.env.action_space())
        assert done
        assert reward == 0.5 #different from AlarmReward because mult_for_right_zone is lower here, 1.5 instead of 2
    ...
    def test_reward_right_time_wrong_area(self):
        ...
        obs, reward, done, info = self.env.step(self.env.action_space())
        assert done
        assert reward == 2/3 #different from AlarmReward because mult_for_right_zone is lower here, 1.5 instead of 2
        assert obs.was_alarm_used_after_game_over
    ...
    def test_reward_correct_alarm_only_cascade_right_zone(self):
        ...
        assert done
        assert reward == 1.  # get the max points because cascade linees in right zone
        assert obs.was_alarm_used_after_game_over
    ...
    def test_alarm_reward_linedisconnection_in_window_right_time_bad_zone(self):
        ...
        assert self._is_cascade_in_zone_alarm(disc_lines_in_cascade, zone_alarm) #a line was during the cascade disconnected in the alamr zone.
