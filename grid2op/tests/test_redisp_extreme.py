...
class TestExtremeCurtail(unittest.TestCase):
    ...
    def _aux_test_gen(obsbefore, obsafter, tol=1e-4, min_loss_slack=0.2):
        ...
        slack_tol = max(2. * abs(loss_after - loss_before), min_loss_slack)
        assert slack_variation <= obsbefore.gen_max_ramp_up[slack] + slack_tol, f"{slack_variation = :.2f}MW, way above the ramp up: {obsbefore.gen_max_ramp_up[slack]:.2f}"
        assert slack_variation >= -obsbefore.gen_max_ramp_down[slack] - slack_tol, f"{slack_variation = :.2f}MW, way below the ramp down: {-obsbefore.gen_max_ramp_down[slack]:.2f}"
    ...
    def _aux_compare_with_ref(env, obs, obs_ref, tol=1e-4, min_loss_slack=0.2):
        ...
        slack_tol = max(2. * abs(loss_ref - loss), min_loss_slack)
        assert abs(obs.gen_p[slack_id] - obs.actual_dispatch[slack_id] -  obs_ref.gen_p[slack_id]) <= slack_tol
...
class TestExtremeStorage(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.env.observation_space.observationClass.storage_max_p_absorb[:] = increase_storage
        self.env.observation_space.observationClass.storage_max_p_prod[:] = increase_storage
        ...
        self.storage_ko_down = self.env.action_space({"set_storage": -self.env.storage_max_p_absorb})
        self.storage_ko_up = self.env.action_space({"set_storage": +self.env.storage_max_p_absorb})
        self.storage_ok_down = self.env.action_space({"set_storage": - 0.5 * self.env.storage_max_p_absorb})
        
        self.storage_curtail = self.env.action_space({"set_storage": 0.8 * self.env.storage_max_p_absorb,
                                                      "curtail": [(el, 0.0) for el in np.where(self.env.gen_renewable)[0]]
                                                      })
    
    @staticmethod
    def _aux_test_storage(obsbefore, obsafter, tol=1.1e-2):
        prod_ = obsafter.storage_power < 0.
        consume_ = obsafter.storage_power > 0.
        assert np.all(obsbefore.storage_power[prod_] >= -obsbefore.storage_max_p_prod[prod_])
        assert np.all(obsbefore.storage_power[consume_] <= obsbefore.storage_max_p_absorb[consume_])
        
        prod_ = obsafter.storage_power < 0.
        consume_ = obsafter.storage_power > 0.
        assert np.all(obsafter.storage_power[prod_] >= -type(obsafter).storage_max_p_prod[prod_])
        assert np.all(obsafter.storage_power[consume_] <= type(obsafter).storage_max_p_absorb[consume_])
        ...
        delta_t = obsafter.delta_time * 60.
        energy_to_power = 3600. / delta_t
        storage_power = 1.0 * obsafter.storage_power
        delta_energy = obsafter.storage_charge - obsbefore.storage_charge
        delta_energy[delta_energy < 0.] *= obsbefore.storage_discharging_efficiency[delta_energy < 0.]
        delta_energy[delta_energy > 0.] /= obsbefore.storage_charging_efficiency[delta_energy > 0.]
        assert np.all(np.abs(delta_energy * energy_to_power + obsbefore.storage_loss - storage_power) <= tol)
    ...
    def test_storage_limit_gen_down(self):
        ...
        assert self.env._amount_storage == -amount_storage_first_step + obs2_power_storage
        ...
        obs4, reward, done, info = self.env.step(self.env.action_space())
        assert np.all(obs4.storage_power == 0.)
        assert self.env._amount_storage == 0.
        self._aux_test_storage(obs3, obs4)
        TestExtremeCurtail._aux_test_gen(obs3, obs4, min_loss_slack=4)  # I generate ~40 MW on this grid with storage, losses changes a lot !
        TestExtremeCurtail._aux_compare_with_ref(self.env, obs4, self.obs4_ref, min_loss_slack=4)
    ...
    def test_storage_curtail(self):
        ...
        gen_curt = obs1.gen_renewable & (obs1.gen_p > 0.)
        assert np.all(obs1.gen_p[gen_curt] / obs1.gen_pmax[gen_curt] > self.storage_curtail.curtail[gen_curt])
        # test the energy / power is properly converted
        self._aux_test_storage(obs, obs1)
        # test the generators are ok
        TestExtremeCurtail._aux_test_gen(obs, obs1, min_loss_slack=4)  # I generate ~40 MW on this grid with storage, losses changes a lot !
