...
class Issue245Tester(unittest.TestCase):
    ...
    def test_simulate(self):
        ...
        self.env.set_thermal_limit(2.0 * np.array([162.49806 , 127.40141 , 107.29365 , 110. / 2., 107.90302 ,
                                                   48.58299 , 135.80658 , 434.52097 , 252.74222 , 649.2642  ,
                                                   56.013855, 183.57967 , 324.4969  ,  79.42569 , 319.6616  ,
                                                   100.69238 ,  46.787445,  73.96375 , 895.7249  , 563.3386  ],
                                                  dtype=dt_float)
                                   )
        # perform the 3 steps
        obs = self.env.reset()
        ...
        *_, sim_info = obs.simulate(act)
        assert len(sim_info["exception"]) > 0, "there should be an exception because the action is illegal"
        *_, info = self.env.step(act)
        assert len(info["exception"]) > 0, "there should be an exception because the action is illegal"
