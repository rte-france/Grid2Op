...
class TestNPYChronics(unittest.TestCase):
    ...
    def test_forecast(self):
        ...
        for ts in range(10):
            ...
            assert sim_obs.minute_of_hour == (obs.minute_of_hour + 5) % 60, f"error at iteration {ts}"
        env.close()
    ...
    def test_change_forecast(self):
        ...
        env.chronics_handler.real_data.change_forecasts(self.load_p, self.load_q, self.prod_p, self.prod_v)  # should not affect anything
        for ts in range(10):
            obs, *_ = env.step(env.action_space())
            assert np.all(self.prod_p[1 + ts, :-1] == obs.gen_p[:-1]), f"error at iteration {ts}"
            sim_obs, *_ = obs.simulate(env.action_space())
...
class TestNPYChronicsWithHazards(unittest.TestCase):
    ...
    def test_maintenance_ok(self):
        ...
        for ts in range(8):
            ...
            assert np.all(obs.time_before_cooldown_line == obs_ref.time_before_cooldown_line), f"error at step {ts}"
            sim_obs, *_ = obs.simulate(env.action_space())
            sim_obs_ref, *_ = obs_ref.simulate(env.action_space())
            assert np.all(sim_obs.time_before_cooldown_line == sim_obs_ref.time_before_cooldown_line), f"error at step {ts}"
