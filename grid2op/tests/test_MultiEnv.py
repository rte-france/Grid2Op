...
from grid2op.Environment import MultiEnvironment2
from grid2op.MakeEnv import make
from grid2op.Observation import CompleteObservation
import pdb


class TestBaseMultiEnv(unittest.TestCase):
    ...
    def test_seeding(self):
        ...
        with warnings.catch_warnings():
            ...
            with make("rte_case5_example", test=True) as env:
                envs = [env for _ in range(nb_env)]
                env.seed(2)
                multi_envs1 = BaseMultiEnvironment(envs)
                seeds_1 = multi_envs1.get_seeds()
                multi_envs1.close()
                multi_envs2 = BaseMultiEnvironment(envs)
                seeds_2 = multi_envs2.get_seeds()
                multi_envs2.close()
                env.seed(2)
                multi_envs3 = BaseMultiEnvironment(envs)
...
class TestMultiEnv2(unittest.TestCase):
    def test_creation_multienv(self):
        nb_envs = [1, 1]
        with warnings.catch_warnings():
            warnings.filterwarnings("ignore")
            with make("rte_case5_example", test=True) as env:
                ...
                multi_envs = MultiEnvironment2(envs, nb_envs)
    ...
    def test_seeding(self):
        ...
        with warnings.catch_warnings():
            ...
            with make("rte_case5_example", test=True) as env:
                envs = [env for _ in range(len(nb_envs))]
                env.seed(2)
                multi_envs1 = MultiEnvironment2(envs, nb_envs)
                seeds_1 = multi_envs1.get_seeds()
                multi_envs1.close()
                multi_envs2 = MultiEnvironment2(envs, nb_envs)
                seeds_2 = multi_envs2.get_seeds()
                multi_envs2.close()
                env.seed(2)
                multi_envs3 = MultiEnvironment2(envs, nb_envs)
                seeds_3 = multi_envs3.get_seeds()
                multi_envs3.close()
                assert np.all(seeds_1 == seeds_3)
                assert np.any(seeds_1 != seeds_2)
