...
class TestAlertTrustScoreNoBlackout(unittest.TestCase):
    ...
    def test_assistant_trust_score_no_blackout_no_attack_no_alert(self) -> None : 
        """ When no blackout and no attack occur, and no alert is raised we expect a reward of 0
            until the end of the episode where we have a bonus (here artificially 1)
            ...
            for i in range(env.max_episode_duration()):
                ...
                if info["opponent_attack_line"] is None : 
                    if i == env.max_episode_duration()-1: 
                        ...
                        assert env._reward_helper.template_reward.total_nb_alerts==0
                        ...
                        assert score_min == 0.
                        assert score_max == DEFAULT_PARAMS_TRUSTSCORE["reward_end_episode_bonus"]
                        assert score == 1.
    ...
    def test_assistant_trust_score_no_blackout_no_attack_alert(self) -> None : 
        """ When an alert is raised while no attack / nor blackout occur, we expect a score of 0
            until the end of the episode where we have a 42
        ...
        with make(self.env_nm,
            ...
            for i in range(env.max_episode_duration()):
                ...
                if done: 
                        ...
                        assert env._reward_helper.template_reward.total_nb_alerts==0
                        assert env._reward_helper.template_reward.no_alert_attack_no_blackout==1
                        score_min, score_max = env._reward_helper.template_reward._compute_min_max_reward(
                            env._reward_helper.template_reward.total_nb_attacks)
                        assert score_min == DEFAULT_PARAMS_TRUSTSCORE["reward_min_no_blackout"]
                        assert score_max == DEFAULT_PARAMS_TRUSTSCORE["reward_end_episode_bonus"] + DEFAULT_PARAMS_TRUSTSCORE["reward_max_no_blackout"]
                        assert score == 1., f"error for step {step}: {score} vs 1"
                else : 
        ...
        with make(self.env_nm,
            ...
            for i in range(env.max_episode_duration()):
                ...
                if done:
                    ...
                    assert env._reward_helper.template_reward.total_nb_alerts==0.
                    assert env._reward_helper.template_reward.no_alert_attack_no_blackout==1
                    assert env._reward_helper.template_reward.alert_attack_no_blackout==0
                    assert env._reward_helper.template_reward.no_alert_attack_blackout==0
                    assert env._reward_helper.template_reward.alert_attack_blackout==0
                    score_min, score_max = env._reward_helper.template_reward._compute_min_max_reward(
                        env._reward_helper.template_reward.total_nb_attacks)
                    assert score_min == DEFAULT_PARAMS_TRUSTSCORE["reward_min_no_blackout"] #TODO A vérifier
                    assert score_max == DEFAULT_PARAMS_TRUSTSCORE["reward_end_episode_bonus"] +\
                            DEFAULT_PARAMS_TRUSTSCORE["reward_max_no_blackout"]
                    assert score == 1. #TODO A vérifier
                else : 
            ...
            for i in range(env.max_episode_duration()):
                ...
                if done:
                    ...
                    assert env._reward_helper.template_reward.cumulated_reward==DEFAULT_PARAMS_TRUSTSCORE["reward_end_episode_bonus"] +\
                            DEFAULT_PARAMS_TRUSTSCORE["reward_max_no_blackout"]
                    assert env._reward_helper.template_reward.total_nb_alerts==0.
                    assert env._reward_helper.template_reward.no_alert_attack_no_blackout==2.
                    assert env._reward_helper.template_reward.alert_attack_no_blackout==0
                    assert env._reward_helper.template_reward.no_alert_attack_blackout==0
                    assert env._reward_helper.template_reward.alert_attack_blackout==0
                    score_min, score_max = env._reward_helper.template_reward._compute_min_max_reward(
        ...
        with make(self.env_nm,
            ...
            for i in range(env.max_episode_duration()):
                ...
                if done: 
                    ...
                    assert env._reward_helper.template_reward.total_nb_alerts==1.
                    assert env._reward_helper.template_reward.no_alert_attack_no_blackout==1.
                    assert env._reward_helper.template_reward.alert_attack_no_blackout==1.
                    assert env._reward_helper.template_reward.no_alert_attack_blackout==0
                    assert env._reward_helper.template_reward.alert_attack_blackout==0
                    score_min, score_max = env._reward_helper.template_reward._compute_min_max_reward(
                        env._reward_helper.template_reward.total_nb_attacks)
                    assert score_min == 2 * DEFAULT_PARAMS_TRUSTSCORE["reward_min_no_blackout"] #TODO A vérifier
                    assert score_max == DEFAULT_PARAMS_TRUSTSCORE["reward_end_episode_bonus"] +\
                            2 * DEFAULT_PARAMS_TRUSTSCORE["reward_max_no_blackout"]
                    assert np.allclose(score, 0.70910781) #TODO A vérifier
                else : 
                    assert score == 0
        ...
        with make(self.env_nm,
            ...
            for i in range(env.max_episode_duration()):
                ...
                if done:
                    ...
                    assert env._reward_helper.template_reward.total_nb_alerts==1.
                    assert env._reward_helper.template_reward.no_alert_attack_no_blackout==0.
                    assert env._reward_helper.template_reward.alert_attack_no_blackout==0.
                    assert env._reward_helper.template_reward.no_alert_attack_blackout==1
                    assert env._reward_helper.template_reward.alert_attack_blackout==0
                    score_min, score_max = env._reward_helper.template_reward._compute_min_max_reward(
                        env._reward_helper.template_reward.total_nb_attacks)
                    assert score_min == 2 * DEFAULT_PARAMS_TRUSTSCORE["reward_min_no_blackout"] #TODO A vérifier
    ...
    def test_assistant_trust_score_no_blackout_2_attack_diff_time_no_alert(self) -> None :
        ...
        with make(self.env_nm,
            ...
            for i in range(env.max_episode_duration()):
                ...
                if done:
                    assert score == 44, f"error for step {step}: {score} vs 44"
                    assert env._reward_helper.template_reward.total_nb_attacks==2
    ...
    def test_assistant_trust_score_no_blackout_2_attack_diff_time_2_alert(self) -> None :
        ...
        with make(self.env_nm,
            ...
            for i in range(env.max_episode_duration()):
                ...
                if done:
                    assert score == 40, f"error for step {step}: {score} vs 40"
                    assert env._reward_helper.template_reward.total_nb_attacks==2
    ...
    def test_assistant_trust_score_no_blackout_2_attack_diff_time_alert_first_attack(self) -> None :
        ...
        with make(self.env_nm,
            ...
            for i in range(env.max_episode_duration()):
                ...
                if done:
                    score == 42, f"error for step {step}: {score} vs 42"
                    assert env._reward_helper.template_reward.total_nb_attacks==2
    ...
    def test_assistant_trust_score_no_blackout_2_attack_diff_time_alert_second_attack(self) -> None :
        ...
        with make(self.env_nm,
            ...
            for i in range(env.max_episode_duration()):
                ...
                if done:
                    assert score == 42, f"error for step {step}: {score} vs 42"
                    assert env._reward_helper.template_reward.total_nb_attacks==2
                    assert env._reward_helper.template_reward.cumulated_reward==42
                else : 
                    assert score == 0, f"error for step {step}: {score} vs 0"
    ...
    def test_assistant_trust_score_blackout_attack_no_alert(self) -> None :
        ...
        with make(self.env_nm,
            ...
            for i in range(env.max_episode_duration()):
                ...
                if done:
                    assert score == -10, f"error for step {step}: {score} vs -10"
                    assert env._reward_helper.template_reward.total_nb_attacks==1
    ...
    def test_assistant_trust_score_blackout_attack_raise_good_alert(self) -> None :
        ...
        with make(self.env_nm,
            ...
            for i in range(env.max_episode_duration()):
                ...
                if done: 
                    assert score == 2, f"error for step {step}: {score} vs 2"
                    assert env._reward_helper.template_reward.total_nb_attacks==1
    ...
    def test_assistant_trust_score_blackout_2_lines_attacked_different_1_in_window_1_good_alert(self) -> None:
        ...
        with make(self.env_nm,
            ...
            for i in range(env.max_episode_duration()):
                ...
                if done : 
                    assert score == 3, f"error for step {step}: {score} vs 3"
                    assert env._reward_helper.template_reward.total_nb_attacks==2
