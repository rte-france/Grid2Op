...
class GridStateFromFileWithForecastsWithMaintenance(GridStateFromFileWithForecasts):
    ...
    def initialize(self, order_backend_loads, order_backend_prods, order_backend_lines, order_backend_subs,
        ...
        self.line_to_maintenance = list(pd.read_csv(os.path.join(self.path,'lines_in_maintenance.csv'),squeeze=True).values) 

        #frequencies of maintenance
        self.daily_proba_per_month_maintenance = list(pd.read_csv(os.path.join(self.path,'maintenance_daily_proba_per_month.csv'),squeeze=True).values) 
        self.max_daily_number_per_month_maintenance = list(pd.read_csv(os.path.join(self.path,'max_daily_Number_of_Mainteance_per_month.csv'),squeeze=True).values) 
        


        super().initialize(order_backend_loads, order_backend_prods, order_backend_lines, order_backend_subs, names_chronics_to_backend)
    ...
    def _generate_maintenance(self):
        ...
        freq=str(int(self.time_interval.total_seconds()))+"s" #should be in the timedelta frequency format in pandas
        ...
        for idx, day_maintenances in maintenances_df.groupby(maintenances_df.index.date):
            ...
            if(dayOfWeek in range(5)):#only maintenance starting on working days
                ...
                if(n_Generated_Maintenance>maxDailyMaintenance):
                    ...
                    linesInMaintenance=random.choices(linesCurrentlyInMaintenance,k=maxDailyMaintenance)
